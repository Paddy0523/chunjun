...
public class ShortColumnSerializer extends TypeSerializerSingleton<AbstractBaseColumn> {
    ...
    public AbstractBaseColumn deserialize(DataInputView source) throws IOException {
        ...
        if (isNotNull) {
            return new BigDecimalColumn(source.readShort());
        } else {
    ...
    public TypeSerializerSnapshot<AbstractBaseColumn> snapshotConfiguration() {
        return new ShortColumnSerializer.ShortColumnSerializerSnapshot();
    }

    /** Serializer configuration snapshot for compatibility and format evolution. */
    @SuppressWarnings("WeakerAccess")
    public static final class ShortColumnSerializerSnapshot
            extends SimpleTypeSerializerSnapshot<AbstractBaseColumn> {

        public ShortColumnSerializerSnapshot() {
            super(() -> INSTANCE);
        }
    }
}
