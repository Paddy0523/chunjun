...
public class IntColumnSerializer extends TypeSerializerSingleton<AbstractBaseColumn> {

    private static final long serialVersionUID = 1L;

    /** Sharable instance of the StringColumnSerializer. */
    public static final IntColumnSerializer INSTANCE = new IntColumnSerializer();
    ...
    public AbstractBaseColumn deserialize(DataInputView source) throws IOException {
        ...
        if (isNotNull) {
            return new BigDecimalColumn(source.readInt());
        } else {
    ...
    public TypeSerializerSnapshot<AbstractBaseColumn> snapshotConfiguration() {
        return new IntColumnSerializerSnapshot();
    }

    /** Serializer configuration snapshot for compatibility and format evolution. */
    @SuppressWarnings("WeakerAccess")
    public static final class IntColumnSerializerSnapshot
            extends SimpleTypeSerializerSnapshot<AbstractBaseColumn> {

        public IntColumnSerializerSnapshot() {
            super(() -> INSTANCE);
        }
    }
}
